...
def mainTextReader(row, index_path, dataframe):
    ...
    for x in bs.find_all(name='p'):
        ...
        if not main_text_string == r'And we\'ll keep you signed in.' or 'Sign in  or  Register' or 'You need one to watch live TV on any channel or device, and BBC programmes on iPlayer. It’s the law.':
...
def descriptionModifier(bs, row, column, dataframe):
    ...
    try:
        ...
        t = bs.find(name='meta', attrs={'property': 'og:description'})
        description_string = t.attrs.get('content')
        temp_dataframe.iloc[row, 3] = description_string
    return temp_dataframe


def urlModifier(bs, row, column, dataframe):
    temp_dataframe = dataframe
    t_list = bs.find(name='meta', attrs={'property': 'og:url'})
    url_string = t_list.attrs.get('content')
    temp_dataframe.iloc[row, 5] = url_string
    printModifyResult(row, column)
    return temp_dataframe
...
def imageUrlModifier(bs, row, column, dataframe):
    ...
    try:
        ...
        e = bs.find(name='meta', attrs={'property': 'og:image'})
        url_string = e.attrs.get('content')
        temp_dataframe.iloc[row, 6] = url_string
    return temp_dataframe
